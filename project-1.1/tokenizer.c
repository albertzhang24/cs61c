...
size_t SelectToken(char* buffer,
  ...
  */
  ...
  if (buffer[size_read] == '+') {  // + and ++
    ...
    /* YOUR CODE HERE */
    if (isprint(buffer[size_read+1]) || buffer[size_read+1] == 'n' || is_space(buffer[size_read+1]) == 1 || buffer[size_read+1] == 't' || buffer[size_read+1] == '0' || buffer[size_read+1] == '\'') {
      t = create_token(filename);
      t->linenum = *linenum;
    ...
    while (size_read + int_len < size && search) {
      if (is_digit(buffer[size_read + int_len])) {
         ...
         */
        /* YOUR CODE HERE */
        long int token = strtol(buffer, NULL, 10);
        int int_token = (int) token;
        t = create_token(filename);
        t->linenum = *linenum;
        t->type = TOKEN_INTEGER;
        t->data.integer = int_token;
        size_read += int_len; 
        /* FIXME IM NOT CORRECT. */
    ...
    while (size_read + id_len < size && search) {
      if (is_identifier_component(buffer[size_read + id_len])) {
        ...
        if (type != TOKEN_ERR) {
          ...
          /* Handle identifiers */
          //malloc for token contents for data.identifier 
          t = create_token(filename);
          t->linenum = *linenum;
